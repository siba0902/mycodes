# -*- coding: utf-8 -*-
"""
Created on Tue Apr 18 18:19:12 2023

@author: Siba Prasad Mohanty
"""
...
print("----Option Chain &  Data Started---")
...
if not os.path.exists("AutoExcelTraders.xlsm"):
    try:
        wb = xw.Book()
        wb.save("AutoExcelTraders.xlsm")
        wb.close()
    except Exception as e:
        print(f"Error Creating Excel File : {e}")
        sys.exit()
wb = xw.Book("AutoExcelTraders.xlsm")

try:
    wb.sheets('OptionChainBnf')
except:
    wb.sheets.add('OptionChainBnf')
oc = wb.sheets('OptionChainBnf')
...
while True:
    ...
    if inp_symbol is not None:
        try:
            ...
            spot_data = {}
            vix_data = {}
            spot_instrument = "NSE:NIFTY 50" if inp_symbol=="NIFTY" else "BSE:SENSEX" if inp_symbol=="SENSEX" else "NSE:NIFTY FIN SERVICE" if inp_symbol == "FINNIFTY" else (
            ...
            oc.range("d7").value = [["Spot LTP", spot_data["last_price"]],
                                    ...
                                    ["Spot Previous Close", spot_data["ohlc"]["close"]],
